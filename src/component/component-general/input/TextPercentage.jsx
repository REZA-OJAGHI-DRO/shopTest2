...
function TextPercentage({
    ...
    const handleChange = (e) => {
        let inputValue = e.target.value.replace("%", "").trim();

        // الگو برای بررسی اعداد
        const pattern = /^[0-9\u06F0-\u06F9]*$/;

        // بررسی اینکه ورودی فقط عدد است یا خالی است
        if (pattern.test(inputValue) || inputValue === "") {
            // افزودن علامت درصد در صورتی که ورودی عددی باشد
            if (inputValue !== "") {
                inputValue = "%" + inputValue;
            }
            setData(inputValue);
        } else {
            // اگر ورودی معتبر نباشد، مقدار ورودی فعلی حفظ می‌شود بدون تغییر
            setData(data); // نگه داشتن مقدار قبلی اگر کاراکتر نامعتبر بود
        }
    };

    // جلوگیری از وارد شدن حروف
    const handleKeyPress = (e) => {
        const char = String.fromCharCode(e.which);
        const pattern = /^[0-9\u06F0-\u06F9]$/; // فقط اعداد

        // اگر کاراکتر ورودی نامعتبر بود، از تایپ آن جلوگیری کن
            ...
            <div className={`${width} flex flex-wrap gap-2 lg:gap-3`}>
                ...
                >
                    <input
                        ...
                        onKeyPress={handleKeyPress} // افزودن جلوگیری از تایپ حروف
                        value={data == null ? '' : data}
                        type="text"
                        className={`bg-transparent w-full text-right px-4 focus:border-transparent focus:outline-none placeholder:text-center ${styleInput}`}
